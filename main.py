...
def traverse_states_with_transitions(state):
    visited = set()
    queue = deque([state])
    visited.add(state)
    ...
    while queue:
        ...
        for symbol, next_states in current_state.transitions.items():
            for next_state in next_states:
                yield (current_state.name, symbol, next_state.name)
                if next_state not in visited:
                    queue.append(next_state)
                    visited.add(next_state)

        for epsilon_state in current_state.epsilon_transitions:
            yield (current_state.name, 'ε', epsilon_state.name)
...
def export_nfa_to_file2(nfa, output_filename):
    states_transitions = list(traverse_states_with_transitions(nfa.start))
